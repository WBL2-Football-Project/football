from .wireframes.ModalDialog import ModalDialog
...
class AppGui(AppControlInterface, tk.Tk):
    ...
    def _genNewMainCanvasFrame(self):
        ...
        self.mainCanvasFrame = tk.Frame(self)
    ...
    def convertJustifyToUI(self, justify: JustifyEnum) -> Any:
        ...
        if justify == JustifyEnum.LEFT:
            ...
            raise ExceptionUIAbstractInterface(
                f'convertJustifyToUI error: {justify} not recognized')
    ...
    def modalDialog(self, title: str, fun: Callable, data: Dict[str, Any], actions: Dict[str, Callable], parentFrame: Any = None, checkFun: Optional[Callable[[Dict], bool]] = None, *args, **kwargs):
        ...
        class SimpleModalDialog(ModalDialog):
            ...
            def showResults(self,result): # shows the results for every vehicle price offer
                if result==True:
                    ...
                    print(
                        f'SimpleModalDialog result True: {self.resultDataDict}')
        ...
        _dlg = SimpleModalDialog(
            _parentFrame, fun, actions, title, data, checkFun)
...
setattr(AppGui, "chooseRecordFromList", lambda self, *args, **
        kwargs: chooseRecordFromList(*args, **kwargs))
setattr(AppGui, "createDialogYesNo", lambda self, title,
        question: createDialogYesNo(title, question, self))
setattr(AppGui, "dialogForEditPlay", lambda self, *args, **
        kwargs: dialogForEditPlay(*args, **kwargs))
setattr(AppGui, "dialogForNewTeam", lambda self, *args, **
        kwargs: dialogForNewTeam(*args, **kwargs))
setattr(AppGui, "dialogForEditTeam", lambda self, *args, **
        kwargs: dialogForEditTeam(*args, **kwargs))
setattr(AppGui, "dialogForNewUser", lambda self, *args, **
        kwargs: dialogForNewUser(*args, **kwargs))
setattr(AppGui, "displayStatisticsForGroupAndItsGamesScheduled", lambda self, *args,
        **kwargs: displayStatisticsForGroupAndItsGamesScheduled(*args, **kwargs))
setattr(AppGui, "displayStatisticsForPlayoffScheduledGames", lambda self, *args,
        **kwargs: displayStatisticsForPlayoffScheduledGames(*args, **kwargs))
setattr(AppGui, "refereeDialogForNewUser", lambda self, *args, **
        kwargs: refereeDialogForNewUser(*args, **kwargs))
setattr(AppGui, "refereeDialogForUserRights", lambda self, *args, **
        kwargs: refereeDialogForUserRights(*args, **kwargs))
setattr(AppGui, "showErrorMessage", lambda self, title,
        content: showErrorMessage(title, content, self))
setattr(AppGui, "showInfoMessage", lambda self, title,
        content: showInfoMessage(title, content, self))
setattr(AppGui, "dialogForAppLoginOrRegister", lambda self, *args, **
        kwargs: dialogForAppLoginOrRegister(*args, **kwargs))
setattr(AppGui, "userMenu", lambda self, *args, **
        kwargs: userMenu(*args, **kwargs))
setattr(AppGui, "refereeMenu", lambda self, *args, **
        kwargs: refereeMenu(*args, **kwargs))
