...
class ModalDialog(simpledialog.Dialog):
    ...
    def buttonbox(self):
        ...
        """
        self.__onOKButton = tk.Button(
            self, text='OK', width=5, font=(FONT, 10), background=PRIMARY_COLOUR, foreground='#FFFFFF')
        self.__onOKButton.pack(side="left", padx=5, pady=5, ipadx=2, ipady=2)
        self.__onOKButton.bind("<Button-1>", lambda event: self.onOK(event))
        if not self._oneOkButton:
            self.__onCancelButton = tk.Button(self, text='Cancel', width=5, font=(
                FONT, 10), background=SECONDARY_COLOUR, foreground='#FFFFFF')
            self.__onCancelButton.pack(
                side="right", padx=5, pady=5, ipadx=2, ipady=2)
            self.__onCancelButton.bind(
                "<Button-1>", lambda event: self.onCancel(event))
...
if __name__ == "__main__":
    ...
    class SampleSimpleModalDialog(ModalDialog):
        ...
        def showResults(self, result):  # shows the results for every vehicle price offer
            ...
            print(
                f"FROM INSIDE OF CHECKOFFERDIALOG, data {'confirmed: '+self.foo+', '+self.bar if result else 'unconfirmed'}")
        ...
        def body(self, frame):  # designs the window widgets
            ...
            _foo_entry.grid(row=1, column=0, columnspan=2,
                            ipady=10, sticky='ew')
            ...
            _bar_entry.grid(row=3, column=0, columnspan=2,
                            ipady=10, sticky='ew')
    ...
    sampleButton.bind(
        "<Button-1>", lambda event: manageSampleSimpleModalDialog(event))
