...
class SystemController(StateMachine):
    ...
    def __init__(self, dbControl: DBAbstractInterface, appControl: AppControlInterface):
        ...
        if not isinstance(dbControl, DBAbstractInterface):
            raise ExceptionSystemController(
                "dbControl must be a DBAbstractInterface implementation")
        if not isinstance(appControl, AppControlInterface):
            raise ExceptionSystemController(
                "appControl must be a AppControl implementation")

        self.dbControl = dbControl
        self.appControl = appControl
        # default login status (no user identification and no rights to the application and database)
        self.loginStatus: LoginStatus = LoginStatus()
        self.loginStatus.setRightsChangedEventHandler(
            self.appControl.refreshMainWindowView)
    ...
    def defineTeam(self):
        ...
        raise ExceptionUIAbstractInterface(
            f"no {inspect.currentframe().f_code.co_name} method defined")
